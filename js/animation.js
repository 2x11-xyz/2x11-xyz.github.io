...
let sequenceIndex = 0;
let sequenceData = [/* your sequence data */];

// Initialize sequence once
initializeSequence();

function initializeSequence() {
    // ... sequence initialization code ...
}

// Resize event listener without sequence reset
window.addEventListener('resize', () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;

    // Redraw the canvas if necessary without resetting the sequence
    redrawCanvas();
});

function redrawCanvas() {
    // Use the current sequence state to redraw
    // ... redrawing code ...
}

window.addEventListener('load', () => {
    if (gridAnimation) {
        gridAnimation.animationRunning = false;
    }
    gridAnimation = new GridAnimation();
});
